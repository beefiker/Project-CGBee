    ...
    try {
      ...
      if (!($row = $query->fetch(PDO::FETCH_ASSOC))) {
        ...
        for($i=0; $i<count($list_rating); $i++){
            ...
            if(!$list_rating[$i]->genres[1]) $genres = $list_rating[$i]->genres[0];
            else $genres = $list_rating[$i]->genres[0].", ".$list_rating[$i]->genres[1];
        ...
        for($i=0; $i<count($list_like); $i++){
          ...
          if(!$list_like[$i]->genres[1]) $genres = $list_like[$i]->genres[0];
          else $genres = $list_like[$i]->genres[0].", ".$list_like[$i]->genres[1];
          $summary = substr($list_like[$i]->summary,0,140) . "...";
          $cut_summary = str_replace("'","",$summary);
